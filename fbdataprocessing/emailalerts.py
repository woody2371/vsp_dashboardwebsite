...
salesmanDict = {'vincep': 'vince', 'vincel': 'vincel', 'glennh': 'glennh', 'ryang': 'ryang', 'tom': 'tom', 'andrew':'andrewh'}
#salesmanDict = {'vincep': 'tom', 'vincel': 'tom', 'glennh': 'tom', 'ryang': 'tom', 'andrewh': 'tom', 'tom': 'tom'}

def sendEmail(message):
    try:
        username = 'vsprelay@zimbra.secure-iss.com'  
        password = 'LER6%7MHBAV6hLQz@:i4'
        server = smtplib.SMTP('zimbra.secure-iss.com', 25)  
        server.ehlo()
...
def updateEmailCSV(soList):
    with open("C:\Dashboard\Dashboard Website\static\emails\emailDeliveryList.csv", mode='w', newline='') as emailcsv:
        csv_writer = csv.writer(emailcsv)
        csv_writer.writerow(soList)

def loadEmailCSV():
    returnList = []
    with open("C:\Dashboard\Dashboard Website\static\emails\emailDeliveryList.csv", mode='r', newline='') as emailcsv:
